...
type dataTypes = { title: string, desc: string, hot: string };
  ...
  return (
    <Box className='mt-4 border-solid	border-2 rounded-sm mx-auto' sx={{ maxWidth: 1000 }} >
      <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
        ...
        {data.map(({ title, desc, hot }: dataTypes, index: number) => {
          return <>
            <ListItem key={index} alignItems="flex-start" sx={{ height: 200 }}>
              <ListItemText className="flex flex-col h-full"
                primary={title}
                primaryTypographyProps={{ className: "text-lg leading-6 font-medium cursor-pointer hover:bg-gray-100" }}
